...
execute unless score $dynalist.iterator.index dynalist.var matches ..-1 unless score $dynalist.iterator.index dynalist.var >= $dynalist.length dynalist.var run function dynalist.private:operations/data_at/iterate_start
...
execute if score $dynalist.iterator.index dynalist.var >= $dynalist.length dynalist.var run function dynalist.private:operations/data_at/return_failure