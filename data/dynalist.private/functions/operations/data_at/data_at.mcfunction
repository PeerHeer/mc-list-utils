...
execute unless score $dynalist.iterator.index dynalist.var matches ..-1 unless score $dynalist.iterator.index dynalist.var >= $dynalist.length.result dynalist.var run function dynalist.private:operations/data_at/iterate_start

# If the index is invalid, return failure.
execute if score $dynalist.iterator.index dynalist.var matches ..-1 run function dynalist.private:operations/data_at/return_failure
execute if score $dynalist.iterator.index dynalist.var >= $dynalist.length.result dynalist.var run function dynalist.private:operations/data_at/return_failure