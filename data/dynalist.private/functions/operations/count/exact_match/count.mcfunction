...
execute unless score $dynalist.length dynalist.var matches 0 unless score $dynalist.operation.diff dynalist.var = $dynalist.length dynalist.var run function dynalist.private:operations/count/exact_match/diff_neq_length

# Output the result of the operation.
scoreboard players operation $dynalist.result dynalist.out = $dynalist.length dynalist.var
