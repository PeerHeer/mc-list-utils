...
execute if score $dynalist.compare.result dynalist.out matches 0 store success score $dynalist.success dynalist.out run data modify storage dynalist:out List prepend from storage dynalist:in ReplaceData
...
execute if score $dynalist.compare.result dynalist.out matches 0 unless score $dynalist.iterator.length dynalist.var matches 0 run function dynalist.private:operations/replace/replace_last/iterate_end

# If the comparison was unsuccessful, and the list is not empty, go to the next iteration.
execute unless score $dynalist.compare.result dynalist.out matches 0 unless score $dynalist.iterator.length dynalist.var matches 0 run function dynalist.private:operations/replace/replace_last/iterate