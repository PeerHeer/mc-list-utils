...
execute if score $dynalist.compare.result dynalist.var matches 0 store success score $dynalist.operation.success dynalist.var run data modify storage dynalist:out List prepend from storage dynalist:in ReplaceData

# Append the index to the result list.
execute if score $dynalist.compare.result dynalist.var matches 0 if score $dynalist.operation.success dynalist.var matches 1 run scoreboard players add $dynalist.result dynalist.out 1
